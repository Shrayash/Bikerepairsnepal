...
@section('content')
    ...
    <div class="card">
        ...
        @endif
        <div class="card-body">
            <form action="{{ route('admin.news.updates', $news->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control category_select" name="category_id" id="exampleFormControlSelect1">
                                ...
                                @foreach ($newsCategory as $catData)
                                    @if ($news->category_id == $catData->id)
                                        <option value="{{ $catData->id }}" selected>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control region_select" name="region_id" id="exampleFormControlSelect2">
                                ...
                                @foreach ($newsRegion as $regData)
                                    @if ($news->region_id == $regData->id)
                                        <option value="{{ $regData->id }}" selected>
                                            {{ $regData->region_name }}
                                        </option>
                                    @else
                                        <option value="{{ $regData->id }}">
                                            {{ $regData->region_name }}
                                        </option>
                                    @endif
                                @endforeach
                ...
                <div class="form-group">
                    ...
                    <textarea name="description" rows="5" cols="40"
                        class="form-control tinymce-editor">{{ $news->description }}</textarea>
                </div>
                <div class="form-group">
                    <label style="color:black; font-size:1rem">Highlights <span
                            style="color:red;font-size:15px;">&nbsp;*</span></label>
                    <textarea name="highlight" rows="4" cols="40"
                        class="form-control tinymce-editor">{{ $news->highlight }}</textarea>
                </div>
                <div class="form-group">
                    ...
                    <br><br>
                    <img id="blah" src="{{ asset('uploads/news_images/' . $news->image) }}"
